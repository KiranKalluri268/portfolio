 ...
 *  away with the page instead of pinning, and the rest of its 190svh of runway
...
test.describe("the page is handed back when the menu and the entry screen overlap", () => {
  ...
  test("About still pins to the top instead of scrolling away", async ({ page }) => {
    const pinnedAt = await page.evaluate(async () => {
      ...
      // Comfortably inside the pin range rather than at its edge: the panel
      // is `position: sticky`, so it does not start pinning until the 30svh
      // lead-in has scrolled past, and the section is 190svh - 30svh -
      // 100svh (the sticky panel itself) = 60svh of actual pin room after
      // that. 0.6 sits at the middle of that range (0.3 to 0.9).
