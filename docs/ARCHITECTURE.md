...
`/` is always the plain site. While the journey carries one section, a stored preference that promotes the site's own address would hand anyone following a link 28 screens of mostly nothing, so the journey has to be asked for: `/mode?mode=3d` is the door, `/cinematic` is the route itself, and the site menu is the control. Coming through `/mode` hands the decision to the device — the scene's own ladder (reduced motion, WebGL2, then a benchmark it must hold 20fps through) runs on arrival and lands anything it turns away back on `/`.

`src/proxy.ts` (Next 16's rename of `middleware`) does that routing before any page renders, so `/` stays statically prerendered. It reads two cookies: `presentation` holds the choice and persists for a year, and now only drives what the menu shows as current; `journey-unavailable` is session-scoped and records that this device could not run the journey, which is what stops the scene's fallback to `still` from becoming a redirect loop back into it. `?presentation=` in the URL beats both, and `/mode?mode=3d` clears the second — asking by name is a request to try the device again. `src/lib/presentation.ts` holds the shared vocabulary; the client half that writes the cookies is `presentation-client.ts`.
